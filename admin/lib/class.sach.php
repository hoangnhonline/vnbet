...
class sach extends db{
	...
	function Sach_Them(&$loi){
        ...
        $url_image= $this->processData($_POST['url_image']);
		$kd = $this->changeTitle($TenSach);
		...
		if($thanhcong==false){
			...
			$sql = "INSERT INTO sach
					VALUES(NULL,'$TenSach',$idML,$idTG,'$NhaXB','$NamXB','$ThoiGian',$user_id,$thutu,0,'$kd','$url_image')";
	...
	function Sach_Sua($idSach,&$loi){
        ...
        $url = ($url_image != NULL) ? $url_image : $_POST['url_image_old'];
		...
		if($thanhcong==false){
			...
			$sql = "UPDATE sach
					...
					NamXB = '$NamXB',TenSach_KD = '$kd',url_image = '$url'
					WHERE idSach = $idSach";
			mysql_query($sql) or die(mysql_error().$sql);
		}
		return $thanhcong;
