...
class trangchu extends db{
    ...
    function ListTrangTrongMucLuc($idDM){
        $sql = "SELECT idTrang,NoiDung,ThuTu FROM trang WHERE idDM = $idDM  ORDER BY ThuTu ASC";
        $rs = mysql_query($sql) or die(mysql_error());
        return $rs;
    }
    function get_detail_mucluc_bystr($str){
        $rs = mysql_query("SELECT idDM,thutu,DanhMuc,DanhMuc_KD,idTrang FROM danhmuc WHERE idDM IN(".$str.")") or die(mysql_error());
        while($row = mysql_fetch_assoc($rs)){
            $arrReturn[$row['idDM']] = $row; 
        }     
        return $arrReturn;
    }
}
