$(function() {
	...
	$('#btnLoc').click(function(){
		$.getJSON("ajax/json.php", {
			...
			$("#nxb").val(response.nxb);
			$("#namxb").val(response.namxb);
			$("#ten_sach").val(response.ten_sach);
			$("#ten_tacgia").val(response.ten_tacgia);	
			$('#current_page').val(response.idTrang);
				$('.baotrang').hide();							
...
$(function(){
	...
	$('.float_item .btn_control_float').toggle(
		function(){
			$(this).parents('.float_item').stop(true, true).animate({width:'25px'}, 500)
		},
		function(){
			$(this).parents('.float_item').animate({width:'225px'},500);
		}
	)
